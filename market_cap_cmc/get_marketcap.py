...
def getAllCoinMarketCap():
    # 从raw_data_cmc中查
    sql = f'''select sum(market_cap) from pv_allcoin_raw_data_daily_cmc_{year} where date_time_ymd="{current_date_time}" 
            and market_cap>0 and id not in (6801,28167,28040,28043,25967,27929);'''
    cs = conn.cursor()
    cs.execute(sql)
    res = cs.fetchone()
...
def getStablecoinMarketCap():
    # 获取当天所有的稳定币
    cs = conn.cursor()
    sql = f'''select sum(market_cap) from pv_allcoin_raw_data_daily_cmc_{year} where date_time_ymd="{current_date_time}" and JSON_CONTAINS(tags, '"stablecoin"')
            and market_cap>0 and id not in (6801,28167,28040,28043,25967,27929);'''
    cs.execute(sql)
    res = cs.fetchone()
...
def getCoinMarketCapByCmcId(coin):
    # 从raw_data_cmc中查
    sql = f'''select market_cap from pv_allcoin_raw_data_daily_cmc_{year} where id={coin} and date_time_ymd="{current_date_time}";'''
    cs = conn.cursor()
    cs.execute(sql)
    res = cs.fetchone()
    if res is None or res[0] is None:
...
# def fill_item_fun():
#     global current_date_time, year
#     for md in missingData:
#         if md[1] is None or md[2] is None:
#             print("连续时间上的数据有缺失")
#             continue
#         # 获取btc的价格
#         current_date_time = md[0]
#         year = current_date_time.split('-')[0]
#         # 缺失值用前两个的均值代替
#         missing_data = (md[0],
#                 str((toDecimal(md[1][1])+toDecimal(md[2][1]))/2), str((toDecimal(md[1][2])+toDecimal(md[2][2]))/2),
#                 str((toDecimal(md[1][3])+toDecimal(md[2][3]))/2), str((toDecimal(md[1][4])+toDecimal(md[2][4]))/2),
#                 str((toDecimal(md[1][5])+toDecimal(md[2][5]))/2), str((toDecimal(md[1][6])+toDecimal(md[2][6]))/2),
#                 str((toDecimal(md[1][7])+toDecimal(md[2][7]))/2), str((toDecimal(md[1][8])+toDecimal(md[2][8]))/2),
#                 str((toDecimal(md[1][9])+toDecimal(md[2][9]))/2), str((toDecimal(md[1][10])+toDecimal(md[2][10]))/2),
#         )
#
...
if __name__ == '__main__':
    ...
    # print("使用中间值填补有空缺的数据")
    # missing_data = fill_item_fun()
    # print(missing_data)
    # if missing_data is not None:
    #     all_data = pd.concat([all_data,missing_data.T],axis=0)
    all_data.columns = ['date_time', 'btc_marketCap', 'btc_marketCap_percentage', 'eth_marketCap',
                    'eth_marketCap_percentage', 'stablecoin_marketCap', 'stablecoin_marketCap_percentage',
                    'altcoin_marketCap', 'altcoin_marketCap_percentage', 'total_marketCap', 'btc_price']

    all_data.sort_values(by="date_time",inplace=True)
    all_data.to_csv("market_cap_data_cmc.csv",index=False)